...
class windowGUI:
	...
	def blasting(self):
		...
		#同时爆破用户名+密码
		if ("$user$" in self.allData[3]and "$pass$" in self.allData[3]) or ("$user$" in self.allData[5]and "$pass$" in self.allData[5]):
		...
		#只爆破用户名
		elif "$user$" in self.allData[3] or "$user$" in self.allData[5]:
			userNames = self.getUsers()
			encodeUserNames = self.getEncodeUser(self.allData[7],userNames)
		...
		#只爆破密码
		elif "$pass$" in self.allData[3] or "$pass$" in self.allData[5]:
			passwds = self.getPasswds()
			encodePasswds = self.getEncodePass(self.allData[8],passwds)
